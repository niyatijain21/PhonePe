DROP TABLE IF EXISTS edw_adhoc.mis_3_and_6_month_retention_delta
;

CREATE TABLE edw_adhoc.mis_3_and_6_month_retention_delta AS
...
FROM
	(
		...
		FROM
			edw.payment_detail
		WHERE
			CONCAT(`year`,substr(CONCAT('00',`month`),-2)) >= date_format(ADD_MONTHS(CURRENT_DATE(),-8),'yyyyMM')
			AND payment_category IS NOT NULL AND state = 'COMPLETED' AND pmod(flags,2) = 0 AND (nexus_state IS NULL OR nexus_state = 'COMPLETED')
			AND first_txn_date >= ADD_MONTHS(CURRENT_DATE(),-8)
		GROUP BY
...
DELETE FROM 
    edw_shared.mis_3_and_6_month_retention
WHERE 
    CONCAT(`year`,substr(CONCAT('00',`month`),-2)) >= date_format(ADD_MONTHS(CURRENT_DATE(),-8),'yyyyMM')
	AND first_txn_date >= ADD_MONTHS(CURRENT_DATE(),-8)
...
INSERT INTO TABLE edw_shared.mis_3_and_6_month_retention
SELECT 
	aquisition_month_mr,
	metric_name_mr,
	metric_value_mr
FROM 
    edw_adhoc.mis_3_and_6_month_retention_delta
;